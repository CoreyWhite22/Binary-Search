...
// Add this function to handle the search button click
function startSearch() {
    const letterInput = document.getElementById('letterInput');
    const letter = letterInput.value;
    if (letter) {
        searchForAlphabeticalIndex(letter);
        // Reset the input field after search
        letterInput.value = '';

        // Create a new button for refreshing the page
        const refreshButton = document.createElement('button');
        refreshButton.innerText = 'New Search';
        refreshButton.onclick = function() {
            window.location.reload();
        };

        // Append the new button to the body or a specific div if you prefer
        document.body.appendChild(refreshButton);
    } else {
        alert('Please enter a letter to search for.');
    }
}

// Modify the window.onload function to not automatically start a search
window.onload = () => {
...
if (typeof module !== 'undefined') {
    module.exports = { searchForAlphabeticalIndex, startSearch }; // Add startSearch to exports if needed
}